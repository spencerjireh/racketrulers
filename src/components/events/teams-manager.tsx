...
export function TeamsManager({ eventId }: { eventId: string }) {
  const trpc = useTRPC();
  ...
  const createTeam = useMutation(
    trpc.teams.create.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries(trpc.teams.list.queryFilter({ eventId }));
        setShowForm(false);
  ...
  return (
    <Card>
            ...
            <TableBody>
              {teams.map((team) => (
                <TableRow key={team.id}>
                  ...
                  <TableCell>{team._count.categoryTeams}</TableCell>
                  <TableCell>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (
                            ...
                            deleteTeam.mutate({ id: team.id, eventId });
                          }
                        }}
      ...
      <TeamFormDialog
        ...
        onSubmit={(data) =>
          createTeam.mutate({ eventId, ...data })
        }
      ...
      <TeamFormDialog
        ...
        onSubmit={(data) => {
          if (editingTeam) {
            updateTeam.mutate({ id: editingTeam.id, eventId, ...data });
          }
        }}
      ...
      <BulkAddTeamsDialog
        ...
        onSubmit={(teams) => bulkCreate.mutate({ eventId, teams })}
