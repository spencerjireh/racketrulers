...
export function PoolManager({ roundId, eventId }: PoolManagerProps) {
  const trpc = useTRPC();
  ...
  const { data: pools } = useQuery(
    trpc.pools.list.queryOptions({ roundId, eventId })
  );

  const invalidatePools = () => {
    queryClient.invalidateQueries(
      trpc.pools.list.queryFilter({ roundId, eventId })
    );
    queryClient.invalidateQueries(
      trpc.rounds.getById.queryFilter({ id: roundId, eventId })
    );
  };
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => autoAssign.mutate({ roundId, eventId, poolCount })}
            ...
            <Card key={pool.id}>
              <CardHeader className="py-2 px-3 flex flex-row items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => deletePool.mutate({ id: pool.id, eventId })}
                >
                  Delete
                </Button>
              </CardHeader>
